...
export const POST: APIRoute = async (context) => {
  ...
  const { campaign_id, description } = parsed.data;
  ...
  const { data: bgOp, error: bgOpError } = await supabase
    ...
    .insert({
      ...
      status: "pending",
      input_ref: { campaign_id, description },
    })
    .select("id")
    .single();
  ...
  const structuringService = ai.createStructuringService({ campaignId: campaign_id });
  ...
  async function* trackedEvents(): AsyncGenerator<GenerationProgressEvent> {
    ...
    let succeeded = false;
    try {
      for await (const event of structuringService.run({ description, bgOpId: bgOp.id })) {
        yield event;
